...
export async function POST(request: Request) {
  ...
  const card = await findActiveCardByCredential({ instituteId, scanType: "NFC", value: body.nfcUid });

  if (!card) {
    return NextResponse.json({ ok: false, message: "NFC card was not found." }, { status: 404 });
  }
