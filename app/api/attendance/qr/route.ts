...
import { qrAttendanceSchema } from "@/lib/validations";
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(
        { ok: false, message: "Invalid QR attendance payload.", errors: parsed.error.flatten().fieldErrors },
        { status: 422 }
      );
    }
    ...
    const scannedValue = parsed.data.scannedValue ?? parsed.data.token;
    const rateLimit = checkAttendanceScanRateLimit({ request, deviceId, scannedValue, scanType: "qr" });
    ...
    console.error(error);
    return NextResponse.json({ ok: false, message: "Could not process QR attendance." }, { status: 500 });
  }
}
