    ...
    <div className="space-y-6">
      <section className="grid gap-4 md:grid-cols-4">
        {[
          ...
          { label: "Pending dues", value: formatCurrency(pendingAmount), icon: CreditCard, tone: "bg-amber-100 text-amber-700" },
          ...
          <CardContent className="space-y-3">
            {enrollments.map((enrollment) => (
              <div key={enrollment.id} className="flex flex-col gap-3 rounded-2xl border bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Badge variant="outline">{formatCurrency(Number(enrollment.classGroup.course.fee))}</Badge>
              </div>
            ))}
          </CardContent>
        </Card>
