        ...
        <CardContent className="receipt-paper p-8">
            ...
            <div className="overflow-hidden rounded-xl border">
              <table className="w-full text-sm">
                <tbody>
                  ...
                  <ReceiptRow label="Amount" value={formatCurrency(payment.amount.toString())} />
                  <ReceiptRow label="Discount" value={formatCurrency(payment.discount.toString())} />
                  <ReceiptRow label="Paid on this receipt" value={formatCurrency(receipt.amount.toString())} />
                  <ReceiptRow label="Total paid" value={formatCurrency(payment.paidAmount.toString())} />
                  <ReceiptRow label="Balance" value={formatCurrency(payment.balance.toString())} />
                  <ReceiptRow label="Method" value={payment.method?.replaceAll("_", " ").toLowerCase() ?? "Not recorded"} />
                  <ReceiptRow label="Received by" value={receipt.receivedBy ?? payment.receivedBy ?? "ClassCard Pro"} />
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div className="grid gap-6 pt-8 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-dashed p-4 text-right">
              ...
              <div className="mt-10 border-t pt-2 text-sm">ClassCard Pro</div>
            </div>
          </div>

          <p className="mt-8 text-center text-xs text-muted-foreground">
            This receipt is generated by ClassCard Pro and is valid without a physical stamp unless required by the institute.
          </p>
        </CardContent>
      </Card>
    </div>
