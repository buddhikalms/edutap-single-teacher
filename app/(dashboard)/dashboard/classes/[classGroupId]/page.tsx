    ...
    <div className="space-y-6">
      ...
      <section className="glass-panel rounded-2xl p-6 sm:p-8">
        <div className="flex flex-col justify-between gap-6 xl:flex-row xl:items-end">
          <div>
            <Badge variant="secondary">{classGroup.subject.name}</Badge>
            <h2 className="mt-4 text-3xl font-semibold">{classGroup.name}</h2>
            <p className="mt-2 text-sm text-muted-foreground">{classGroup.code} · {classGroup.gradeLevel?.name ?? "Grade not set"}</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <Metric icon={CreditCard} label="Fee" value={formatCurrency(classGroup.monthlyFee?.toString() ?? 0, currency)} />
          </div>
        </div>
      </section>
