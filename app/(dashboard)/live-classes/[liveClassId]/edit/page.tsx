...
export default async function EditLiveClassPage({ params }: PageProps) {
  ...
  const [liveClass, classes, courses, teachers, students] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      <LiveClassForm action={action} classes={classes} courses={courses} teachers={teachers} students={students} liveClass={liveClass} />
    </div>
  );
}
