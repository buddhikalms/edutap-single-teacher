...
type ApiRequestOptions = { method?: string; body?: unknown; token?: string; apiUrl?: string };
...
export async function apiFormRequest<T>(path: string, formData: FormData, options: { method?: string; token?: string; apiUrl?: string } = {}) {
  const { apiUrl, token } = await getRequestContext(options);
  const response = await fetch(`${apiUrl}${path}`, {
